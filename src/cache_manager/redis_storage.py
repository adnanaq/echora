"""
Custom Redis storage backend for Hishel HTTP caching.

Implements SyncBaseStorage interface for Redis-backed HTTP response caching
...
class SyncRedisStorage(SyncBaseStorage):
    """
    Redis-backed storage for Hishel HTTP caching.
    ...
    Redis Key Structure:
        cache:entry:{uuid}          → Hash with serialized Entry
        cache:stream:{uuid}         → List of response chunks
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            client: Existing Redis client (optional)
    ...
    def create_entry(
        ...
        assert isinstance(response.stream, Iterator)
    ...
    def get_entries(self, key: str) -> List[Entry]: 
        ...
        """
        index_key = self._index_key(key)
        entry_ids_bytes = self.client.smembers(index_key)
        ...
        for entry_id_bytes in entry_ids_bytes:
            ...
            # Get entry data
            entry_hash = self.client.hgetall(entry_key)
            ...
            if self.refresh_ttl_on_access:
                ...
                if ttl is not None:
                    self.client.expire(entry_key, int(ttl))
                    self.client.expire(self._stream_key(entry_id), int(ttl))
    ...
    def _save_stream(
        self, stream: Iterator[bytes], entry_id: uuid.UUID
    ) -> Iterator[bytes]:
        """
        Wrapper around iterator that saves response stream to Redis in chunks.
        ...
        for chunk in stream:
            ...
            self.client.rpush(stream_key, chunk)
            yield chunk

        # Mark stream as complete
        self.client.rpush(stream_key, self._COMPLETE_CHUNK_MARKER)

    def _stream_data_from_cache(self, entry_id: uuid.UUID) -> Iterator[bytes]:
        """
        Get iterator that yields response stream from Redis.
        ...
        chunks = self.client.lrange(stream_key, 0, -1)
    ...
    def cleanup_expired(self) -> int:
        ...
        while True:
            ...
            for key in keys:
                entry_hash = self.client.hgetall(key)
                if not entry_hash or b"data" not in entry_hash:
                    continue
                ...
                if self.is_soft_deleted(entry) and self.is_safe_to_hard_delete(entry): 
                    self._hard_delete_entry(entry.id) 
                    cleaned += 1
    ...
    def _hard_delete_entry(self, entry_id: uuid.UUID) -> None:
        """
