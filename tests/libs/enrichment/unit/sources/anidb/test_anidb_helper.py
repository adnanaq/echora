...
async def test_make_single_request_success(helper, mock_session) -> None:
    ...
    result = await helper._make_single_request({"request": "anime", "aid": 1}, attempt=0)
...
async def test_make_single_request_gzip(helper, mock_session) -> None:
    ...
    mock_session.get.return_value.__aenter__.return_value.read = AsyncMock(return_value=gzipped)
    helper.session = mock_session
    result = await helper._make_single_request({"request": "anime", "aid": 2}, attempt=0)
    assert result == "<anime id='2'></anime>"


@pytest.mark.asyncio
@pytest.mark.parametrize("status_code", [503, 404])
async def test_make_single_request_http_errors(helper, mock_session, status_code) -> None:
...
async def test_fetch_anime_returns_none_on_invalid_url(helper) -> None:
    anime_dict, anime_model = await helper._fetch_anime("https://anidb.net/character/40")
...
async def test_fetch_episodes_returns_regular_only(helper) -> None:
    ...
    model = AniDBAnime(
        ...
        episodes=[
            ...
            AniDBEpisode(id=3, episode_type=1, episode_number="S1"),  # string ep → excluded
        ],
    )
...
@pytest.mark.asyncio
async def test_fetch_all_orchestrates_anime_and_episodes(helper, tmp_path: Path) -> None:
...
async def test_fetch_all_skips_episodes_when_false(helper) -> None:
    ...
    result = await helper.fetch_all(
        {"anidb_url": _ANIDB_URL}, {}, fetch_episodes=False
    )
