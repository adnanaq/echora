...
async def test_main_function_success(mock_helper_class, tmp_path):
    ...
    with patch(
        "sys.argv", ["script.py", "https://kitsu.app/anime/123", "--output", output]
    ):
    ...
    mock_helper.fetch_all.assert_awaited_once_with(
        {"kitsu_url": "https://kitsu.app/anime/123"}, {}
    )
...
async def test_main_function_no_data_found(mock_helper_class, tmp_path):
    ...
    output = str(tmp_path / "output.json")
    with patch(
        "sys.argv", ["script.py", "https://kitsu.app/anime/99999", "--output", output]
    ):
        exit_code = await main()
...
async def test_paginate_sleeps_on_live_multi_page():
    ...
    with patch(
        "enrichment.sources.kitsu.kitsu_helper.asyncio.sleep", new_callable=AsyncMock
    ) as mock_sleep:
        items, _ = await helper._paginate("/anime/1/episodes")
...
async def test_fetch_characters_skips_chars_without_character_object():
    ...
    from enrichment.sources.kitsu.kitsu_helper import KitsuHelper
    from enrichment.sources.kitsu.kitsu_models import KitsuMediaCharacter
...
async def test_fetch_all_slug_resolution_no_data():
    ...
    helper = KitsuHelper()
    helper._make_request = AsyncMock(return_value={"data": [], "_from_cache": False})
