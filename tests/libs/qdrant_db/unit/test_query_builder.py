...
from qdrant_client.models import FieldCondition, Filter, MatchAny, MatchValue, Prefetch, Range, SparseVector
...
def test_build_filter_eq_operator() -> None:
    result = build_filter([SearchFilterCondition(field="type", operator="eq", value="anime")])
...
def test_build_filter_range_operator() -> None:
    result = build_filter(
        [SearchFilterCondition(field="year", operator="range", value={"gte": 2000, "lte": 2020})]
    )
    assert isinstance(result, Filter)
    cond = result.must[0]
    assert isinstance(cond, FieldCondition)
