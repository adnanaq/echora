    ...
    for anime in anime_data:
        if anime.get("staff_data") and anime["staff_data"].get("production_staff"):
            ...
            for key in staff.keys():
                if (
                    isinstance(staff[key], list) and staff[key]
                ):  # Has actual staff members
                    all_roles.add(key)
...
def test_staff_vector_comprehensive():
    ...
    roles_display = ", ".join(
        [role.replace("_", " ").title() for role in staff_roles[:10]]
    )
    ...
    for i, anime in enumerate(test_anime):
        ...
        staff_names = extract_staff_names_from_query(
            text_query, anime, selected_combination
        )
        ...
        try:
            ...
            results = asyncio.run(
                qdrant_client.search_single_vector(
                    vector_name="staff_vector", vector_data=embedding, limit=5
                )
            )
            ...
            if results:
                ...
                if top_title == anime_title:
                    ...
                    test_passed = True
                    passed_tests += 1
                elif staff_names and verify_staff_in_anime(
                    staff_names, top_title, anime_database
                ):
                    test_passed = True
                    passed_tests += 1
    ...
    if pattern_stats:
        ...
        for pattern_name, stats in pattern_stats.items():
            if stats["tests"] > 0:
                ...
                print(
                    f"   • {pattern_name}: {stats['passes']}/{stats['tests']} ({success_rate:.1f}%) | Avg Score: {avg_score:.4f}"
                )
            else:
    ...
    formatter.print_final_results(
        "Staff Vector Test", passed_tests, total_tests, random_seed, insights
    )
