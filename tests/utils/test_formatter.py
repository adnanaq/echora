...
class TestResultFormatter:
    ...
    def print_test_summary(self, test_name: str, total_anime: int, test_count: int,
                          field_combinations: int, random_seed: int):
        ...
        self.console.print(Panel(table, title=f"[bold]{test_name} Configuration[/bold]",
                               border_style="blue"))
    ...
    def print_detailed_test_result(self, test_num: int, anime_data: Dict[str, Any],
                                  field_combo: List[str], query_text: str,
                                  results: List[Dict[str, Any]], test_passed: bool,
        ...
        field_panel_content = []
        title_fields = ['title', 'title_english', 'title_japanese', 'synonyms', 'synopsis', 'background']
            ...
            if value:
                if field == 'synonyms':
                    if isinstance(value, list) and len(value) > 0:
                        ...
                        field_panel_content.append(f"[cyan]{field}:[/cyan] {synonyms_preview}")
                elif field in ['synopsis', 'background']:
                    text_value = str(value)
                    char_count = len(text_value)
                    preview = text_value[:100] + ("..." if char_count > 100 else "")
                    field_panel_content.append(f"[cyan]{field}:[/cyan] {preview} [dim]({char_count} characters)[/dim]")
                else:
    ...
    def print_detailed_image_test_result(self, test_num: int, anime_data: Dict[str, Any],
                                        image_url: str, available_images: int,
                                        embedding_dim: int, results: List[Dict[str, Any]],
                                        test_passed: bool):
        ...
        # Create the header separator
        header = f"Image Test #{test_num} " + "═" * (75 - len(f"Image Test #{test_num} "))
        ...
        image_panel_content = []
        image_panel_content.append(f"[bold blue]Available images:[/bold blue] {available_images}")
        image_panel_content.append(f"[blue]Selected image:[/blue] {image_url}")
        image_panel_content.append(f"[bold green]Embedding dimension:[/bold green] {embedding_dim}D (OpenCLIP ViT-L/14)")
        ...
        if 'images' in anime_data:
            ...
            if image_types:
                image_panel_content.append(f"[cyan]Image types:[/cyan] {' | '.join(image_types)}")
        ...
        for i, result in enumerate(results[:5], 1):
            ...
            score_style = "bright_green" if result_score >= 0.9 else "yellow" if result_score >= 0.7 else "red" if result_score >= 0.5 else "dim"
            score_display = f"[{score_style}]{result_score:.4f}[/{score_style}]"
        ...
        if results:
            ...
            if test_passed:
                if top_score >= 0.95:
                    analysis_text = "EXCELLENT VISUAL MATCH - Nearly perfect image similarity!"
                elif top_score >= 0.8:
                    analysis_text = "STRONG VISUAL MATCH - High image similarity detected!"
                else:
                    analysis_text = "VISUAL MATCH - Image correctly identified source anime!"
            else:
            ...
            if results and len(results) >= 2:
                ...
                if confidence_gap > 0.3:
                    ...
                    confidence_text = "Lower confidence - similar visual features detected"

                self.console.print(f"   [dim]Confidence gap: {confidence_gap:.4f} - {confidence_text}[/dim]")
    ...
    def print_field_combination_analysis(self, field_stats: Dict[str, Dict[str, Any]]):
        ...
        for combo_key, stats in sorted_stats:
            if stats["tests"] > 0:
                ...
                combo_display = combo_key[:18] + "..." if len(combo_key) > 18 else combo_key

                # Success rate styling - more compact
                rate_style = "bright_green" if success_rate >= 80 else "yellow" if success_rate >= 60 else "red"
                rate_display = f"[{rate_style}]{success_rate:.0f}%[/{rate_style}]"

                # Score styling - more compact
                score_style = "bright_green" if avg_score >= 0.7 else "yellow" if avg_score >= 0.4 else "red"
                score_display = f"[{score_style}]{avg_score:.3f}[/{score_style}]"

                # Fusion boost if available - more compact
                fusion_display = ""
                if "fusion_boost" in stats:
                    fusion_rate = (stats["fusion_boost"] / stats["tests"]) * 100 if stats["tests"] > 0 else 0
                    fusion_style = "bright_green" if fusion_rate >= 70 else "yellow" if fusion_rate >= 40 else "dim"
                    fusion_display = f"[{fusion_style}]{fusion_rate:.0f}%[/{fusion_style}]"
    ...
    def print_final_results(self, test_name: str, passed_tests: int, total_tests: int,
                           random_seed: int, insights: List[str] = None):
        ...
        results_table.add_row("✅ Tests passed:", f"[bold green]{passed_tests}/{total_tests}[/bold green]")

        # Success rate with styling
        if success_rate >= 80:
            rate_display = f"[bold bright_green]{success_rate:.1f}%[/bold bright_green] 🎉"
        ...
        border_style = "bright_green" if success_rate >= 80 else "yellow" if success_rate >= 60 else "red"
    ...
    def print_image_test_details(self, test_num: int, anime_title: str,
                                image_count: int, image_url: str, embedding_dim: int):
    ...
    def print_multimodal_test_details(self, test_num: int, anime_title: str,
                                     field_combo: str, text_query: str,
                                     text_score: float, image_score: float,
                                     multimodal_score: float, fusion_boost: bool):
        """Print multimodal test details with score analysis."""
        details = f"🎬 Multimodal Test {test_num}: [bold cyan]{anime_title}[/bold cyan]\n"
        ...
        text_style = "bright_green" if text_score >= 0.7 else "yellow" if text_score >= 0.4 else "red"
        image_style = "bright_green" if image_score >= 0.7 else "yellow" if image_score >= 0.4 else "red"
        multi_style = "bright_green" if multimodal_score >= 0.7 else "yellow" if multimodal_score >= 0.4 else "red"

        details += f"   📈 Scores: Text=[{text_style}]{text_score:.4f}[/{text_style}] | "
    ...
    def print_final_summary(self, title: str, passed: int, total: int,
                          success_rate: float, status_msg: str,
                          status_color: str, random_seed: int):
    ...
    def print_detailed_multimodal_test_result(self, test_num: int, anime_data: Dict[str, Any],
                                             selected_combination: List[str], text_query: str,
                                             image_url: str, available_images: int,
                                             text_score: float, image_score: float,
                                             multimodal_score: float, multimodal_results: List[Dict[str, Any]],
                                             test_passed: bool, fusion_boost: bool):
        ...
        # Create the header separator
        header = f"Multimodal Test #{test_num} " + "═" * (75 - len(f"Multimodal Test #{test_num} "))
        self.console.print(f"\n[bold]{header}[/bold]")
        ...
        field_panel_content = []
        field_panel_content.append(f"[bold yellow]Selected combination:[/bold yellow] {combination_key}")
        field_panel_content.append(f"[bold blue]Available images:[/bold blue] {available_images}")
        ...
        for field in selected_combination:
            ...
            if value:
                if field == 'synonyms':
                    ...
                    field_panel_content.append(f"   {field}: {len(str(value))} characters")
                else:
        ...
        query_display = text_query[:100] + "..." if len(text_query) > 100 else text_query
        query_panel = Panel(
            f"[cyan]{query_display}[/cyan]",
            title="📝 Generated Text Query",
            border_style="cyan",
        ...
        score_content += f"[{image_style}]Image-only:[/{image_style}] {image_score:.4f}\n"
        score_content += f"[{multi_style}]Multimodal (RRF):[/{multi_style}] {multimodal_score:.4f}"
        ...
        if test_passed:
            if anime_title == multimodal_results[0].get("title", ""):
                analysis_text = "EXACT MULTIMODAL MATCH - Perfect fusion of text and image vectors!"
            else:
        ...
        if fusion_boost:
            ...
            fusion_text = "🎯 FUSION BENEFIT - Multimodal improved over individual results"
            fusion_style = "green"
        else:
            fusion_text = "⚠️ LIMITED FUSION - Individual vectors performed better"
            fusion_style = "yellow"

        self.console.print(f"   [dim]Fusion effectiveness: [{fusion_style}]{fusion_text}[/{fusion_style}][/dim]")
