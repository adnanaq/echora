...
async def add_test_anime(
    ...
    for anime in anime_list:
        ...
        documents.append(
            VectorDocument(id=point_id, vectors={}, payload=anime.model_dump())
        )
...
async def test_detailed_results_provide_accurate_tracking(
    ...
    title_content2 = embedding_manager.field_mapper._extract_title_content(
        test_anime[2]
    )
...
async def test_all_or_nothing_anime_success_logic(
    ...
    character_content = embedding_manager.field_mapper._extract_character_content(
        test_anime
    )
...
@pytest.mark.asyncio
async def test_all_11_vectors_simultaneously(
    client: QdrantClient, embedding_manager: MultiVectorEmbeddingManager
):
...
async def test_batch_size_boundaries(
    ...
    for batch_size in batch_sizes_to_test:
        ...
        for i in range(batch_size):
            content = embedding_manager.field_mapper._extract_title_content(
                test_anime[i]
            )
            vector = embedding_manager.text_processor.encode_text(content)
            batch_updates.append(
