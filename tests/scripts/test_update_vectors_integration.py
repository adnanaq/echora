...
async def test_vector_persistence_after_update(client: QdrantClient):
    ...
    title_content = client.embedding_manager.field_mapper._extract_title_content(
        test_anime
    )
    ...
    query_vector = client.embedding_manager.text_processor.encode_text(
        "Persistence Test Anime"
    )

    search_results = await client.search_single_vector(
        vector_name="title_vector", vector_data=query_vector, limit=5
    )
...
async def test_detailed_results_provide_accurate_tracking(client: QdrantClient):
    ...
    title_content2 = client.embedding_manager.field_mapper._extract_title_content(
        test_anime[2]
    )
...
async def test_all_or_nothing_anime_success_logic(client: QdrantClient):
    ...
    character_content = (
        client.embedding_manager.field_mapper._extract_character_content(test_anime)
    )
...
async def test_per_vector_statistics_from_detailed_results(client: QdrantClient):
    ...
    for i, anime in enumerate(test_anime):
        ...
        title_vector = client.embedding_manager.text_processor.encode_text(
            title_content
        )
        batch_updates.append(
            {
        ...
        if i == 1:
            ...
            genre_content = (
                client.embedding_manager.field_mapper._extract_genre_content(anime)
            )
...
async def test_all_11_vectors_simultaneously(client: QdrantClient):
    ...
    vector_names = [
        ...
        "episode_vector",
        "image_vector",
        "character_image_vector",  # Image vectors (768-dim)
    ]
...
async def test_dimension_edge_cases(client: QdrantClient):
    ...
    batch_updates = [
        ...
        # One less than expected
        {
            "anime_id": test_anime.id,
            "vector_name": "character_vector",
            "vector_data": [0.1] * 1023,
        },
        # One more than expected
        {
            "anime_id": test_anime.id,
            "vector_name": "staff_vector",
            "vector_data": [0.1] * 1025,
        },
        # Exactly correct
        {
            "anime_id": test_anime.id,
            "vector_name": "temporal_vector",
            "vector_data": [0.1] * 1024,
        },
    ]
    ...
    dimension_errors = [
        r
        for r in result["results"]
        if not r["success"] and "dimension" in r.get("error", "").lower()
    ]
    validation_errors = [
        r
        for r in result["results"]
        if not r["success"] and "valid" in r.get("error", "").lower()
    ]
...
async def test_special_float_values(client: QdrantClient):
    ...
    batch_updates = [
        # Very small values (near zero)
        {
            "anime_id": test_anime.id,
            "vector_name": "title_vector",
            "vector_data": [1e-10] * 1024,
        },
        # Very large values
        {
            "anime_id": test_anime.id,
            "vector_name": "genre_vector",
            "vector_data": [1e10] * 1024,
        },
        # Negative values
        {
            "anime_id": test_anime.id,
            "vector_name": "character_vector",
            "vector_data": [-0.5] * 1024,
        },
        # Zero values
        {
            "anime_id": test_anime.id,
            "vector_name": "staff_vector",
            "vector_data": [0.0] * 1024,
        },
        # Normal values
        {
            "anime_id": test_anime.id,
...
async def test_results_ordering_matches_input(client: QdrantClient):
    ...
    for i, anime in enumerate(test_anime):
        ...
        batch_updates.append(
            {"anime_id": anime.id, "vector_name": "title_vector", "vector_data": vector}
        )
...
async def test_update_then_search_consistency(client: QdrantClient):
    ...
    search_results = await client.search_single_vector(
        vector_name="title_vector", vector_data=title_vector, limit=5
    )
...
async def test_similarity_search_after_multiple_updates(client: QdrantClient):
    ...
    different_result = next(
        (r for r in search_results if r["id"] == "different-1"), None
    )
    if different_result:
        assert (
