...
class TestKitsuDataLoading:
    ...
    def test_load_kitsu_episode_data_empty_thumbnail(self, tmp_path):
        ...
        data = {
            ...
            "episodes": [
                {"attributes": {"number": 1, "thumbnail": {}}},  # Empty thumbnail
                {
                    "attributes": {
                        "number": 2,
    ...
    def mock_anisearch_data(self):
        ...
        return {
            "episodes": [
                ...
                {"episodeNumber": 3, "title": ""},  # Empty title should be skipped
                {
...
class TestEpisodeProcessing:
    ...
    def mock_episodes_detailed(self):
        """Create mock mal_episodes.jsonl data."""
        return [
            {
                "episode_number": 1,
                "title": "Jikan Title 1",
...
class TestEdgeCases:
    ...
    def test_missing_episodes_detailed_file(self, tmp_path):
        """Test handling when mal_episodes.jsonl is missing."""
...
class TestMainExecution:
    ...
    def test_main_with_agent_id_absolute_path(self, tmp_path):
        ...
        episodes_file = anime_dir / "mal_episodes.jsonl"
        with open(episodes_file, "w") as f:
            json.dump(
                [
                    {
    ...
    def test_main_with_missing_episodes_file(self, tmp_path):
        """Test main execution when mal_episodes.jsonl is missing."""
        anime_dir = tmp_path / "test_agent"
        anime_dir.mkdir()
