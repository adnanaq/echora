...
async def seeded_test_data(client: QdrantClient, embedding_manager, test_anime_data: List[Dict]):
...
@pytest.mark.asyncio
async def test_update_single_vector(client: QdrantClient, embedding_manager, seeded_test_data: List[AnimeEntry]):
...
@pytest.mark.asyncio
async def test_update_single_vector_invalid_name(client: QdrantClient, embedding_manager, seeded_test_data: List[AnimeEntry]):
...
@pytest.mark.asyncio
async def test_update_batch_vectors(client: QdrantClient, embedding_manager, seeded_test_data: List[AnimeEntry]):
    ...
    assert len(result['results']) == len(seeded_test_data), "Should have result for each update"

    # Verify all detailed results show success
    for update_result in result['results']:
        assert update_result['success'] is True, f"Update should succeed: {update_result}"
...
@pytest.mark.asyncio
async def test_update_batch_vectors_mixed(client: QdrantClient, embedding_manager, seeded_test_data: List[AnimeEntry]):
...
@pytest.mark.asyncio
async def test_update_image_vector(client: QdrantClient, embedding_manager, seeded_test_data: List[AnimeEntry]):
    """Test updating image vector."""
    anime = seeded_test_data[0]

    # Generate image vector
    image_vector = await embedding_manager.vision_processor.process_anime_image_vector(anime)
...
@pytest.mark.asyncio
async def test_update_all_text_vectors(client: QdrantClient, embedding_manager, seeded_test_data: List[AnimeEntry]):
...
@pytest.mark.asyncio
async def test_update_batch_vectors_with_failures(client: QdrantClient, embedding_manager, seeded_test_data: List[AnimeEntry]):
    ...
    assert 'invalid_vector_name' in str(failed_updates[0]['error']).lower(), "Error should mention invalid vector"


@pytest.mark.asyncio
async def test_update_batch_vectors_all_validation_failures(client: QdrantClient, embedding_manager, seeded_test_data: List[AnimeEntry]):
...
@pytest.mark.asyncio
async def test_update_batch_vectors_partial_anime_success(client: QdrantClient, embedding_manager, seeded_test_data: List[AnimeEntry]):
    ...
    assert failed_result['vector_name'] == 'genre_vector', "Should identify genre_vector as failed"
    assert 'dimension' in failed_result['error'].lower(), "Error should mention dimension mismatch"


@pytest.mark.asyncio
async def test_update_batch_vectors_multiple_anime_mixed_results(client: QdrantClient, embedding_manager, seeded_test_data: List[AnimeEntry]):
...
@pytest.mark.asyncio
async def test_update_batch_vectors_dimension_validation(client: QdrantClient, embedding_manager, seeded_test_data: List[AnimeEntry]):
    ...
    dimension_errors = [r for r in result['results'] if 'dimension' in r['error'].lower()]
    validation_errors = [r for r in result['results'] if 'valid' in r['error'].lower()]

    assert len(dimension_errors) >= 2, "Should have at least 2 dimension mismatch errors"
    assert len(validation_errors) >= 1, "Empty vector should fail validation check"


@pytest.mark.asyncio
async def test_update_batch_vectors_invalid_data_types(client: QdrantClient, embedding_manager, seeded_test_data: List[AnimeEntry]):
...
@pytest.mark.asyncio
async def test_update_batch_vectors_same_vector_multiple_updates(client: QdrantClient, embedding_manager, seeded_test_data: List[AnimeEntry]):
    ...
    title_vector2 = embedding_manager.text_processor.encode_text(title_content)  # Same content, different instance
    ...
    assert result['results'][0]['vector_name'] == 'title_vector', "Should be title_vector"
    assert result['results'][0]['anime_id'] == anime.id, "Should be correct anime_id"


@pytest.mark.asyncio
async def test_update_batch_vectors_large_batch(client: QdrantClient, embedding_manager, seeded_test_data: List[AnimeEntry]):
    ...
    assert result_vector_names == set(text_vector_names), "All vectors should be in results"


@pytest.mark.asyncio
async def test_update_batch_vectors_preserves_order(client: QdrantClient, embedding_manager, seeded_test_data: List[AnimeEntry]):
    ...
    batch_updates = [
        {'anime_id': anime.id, 'vector_name': 'title_vector', 'vector_data': title_vector},  # Valid
        {'anime_id': anime.id, 'vector_name': 'invalid_1', 'vector_data': title_vector},     # Invalid
        {'anime_id': anime.id, 'vector_name': 'genre_vector', 'vector_data': title_vector},  # Valid
...
async def test_deduplication_last_wins_policy(client: QdrantClient, embedding_manager):
    ...
    batch_updates = [
        {'anime_id': anime.id, 'vector_name': 'title_vector', 'vector_data': first_vector},
    ...
    assert our_anime['similarity_score'] > 0.99, "Should match the last vector with high similarity"
...
async def test_deduplication_first_wins_policy(client: QdrantClient, embedding_manager):
    ...
    our_anime = next((r for r in search_result if r['id'] == anime.id), None)
    assert our_anime is not None, f"Should find anime {anime.id} in results"
    assert our_anime['similarity_score'] > 0.99, "Should match the first vector with high similarity"
...
async def test_deduplication_fail_policy(client: QdrantClient, embedding_manager):
    ...
    batch_updates = [
        ...
        {'anime_id': anime.id, 'vector_name': 'title_vector', 'vector_data': [0.9] * 1024},  # Duplicate
    ]
...
@pytest.mark.asyncio
async def test_deduplication_warn_policy(client: QdrantClient, embedding_manager, caplog):
...
async def test_no_duplicates_all_policies(client: QdrantClient, embedding_manager):
    ...
    batch_updates = [
        {'anime_id': anime.id, 'vector_name': 'title_vector', 'vector_data': [0.1] * 1024},
        {'anime_id': anime.id, 'vector_name': 'genre_vector', 'vector_data': [0.2] * 1024},
    ]
...
async def test_retry_on_transient_error(client: QdrantClient, embedding_manager):
    ...
    with patch.object(client.client, 'update_vectors', side_effect=mock_update_with_transient_failure):
        batch_updates = [
            {'anime_id': anime.id, 'vector_name': 'title_vector', 'vector_data': [0.1] * 1024}
        ]
...
async def test_max_retries_exceeded(client: QdrantClient, embedding_manager):
    ...
    from unittest.mock import patch
    with patch.object(client.client, 'update_vectors', side_effect=mock_update_always_fails):
        batch_updates = [
            {'anime_id': anime.id, 'vector_name': 'title_vector', 'vector_data': [0.1] * 1024}
        ]
    ...
    assert "failed after" in result['results'][0]['error'].lower(), "Error should mention retry attempts"
...
async def test_non_transient_error_no_retry(client: QdrantClient, embedding_manager):
    ...
    from unittest.mock import patch
    with patch.object(client.client, 'update_vectors', side_effect=mock_update_non_transient_error):
        batch_updates = [
            {'anime_id': anime.id, 'vector_name': 'title_vector', 'vector_data': [0.1] * 1024}
        ]
...
@pytest.mark.asyncio
async def test_update_batch_anime_vectors_basic(client: QdrantClient, embedding_manager):
...
@pytest.mark.asyncio
async def test_update_batch_anime_vectors_all_vectors(client: QdrantClient, embedding_manager, settings):
...
@pytest.mark.asyncio
async def test_update_batch_anime_vectors_with_progress_callback(client: QdrantClient, embedding_manager):
...
@pytest.mark.asyncio
async def test_update_batch_anime_vectors_handles_generation_failures(client: QdrantClient, embedding_manager):
    ...
    generation_failures = [v for v in requested_vectors if v not in vectors or not vectors[v]]
...
@pytest.mark.asyncio
async def test_update_batch_anime_vectors_empty_list(client: QdrantClient, embedding_manager):
...
@pytest.mark.asyncio
async def test_update_batch_anime_vectors_respects_batch_size(client: QdrantClient, embedding_manager):
...
@pytest.mark.asyncio
async def test_update_single_anime_vector_success(client: QdrantClient, embedding_manager):
...
@pytest.mark.asyncio
async def test_update_single_anime_vector_invalid_vector_name(client: QdrantClient, embedding_manager, settings):
...
@pytest.mark.asyncio
async def test_update_single_anime_vector_generation_failure(client: QdrantClient, embedding_manager):
...
@pytest.mark.asyncio
async def test_update_single_anime_vector_multiple_vectors_sequential(client: QdrantClient, embedding_manager):
...
@pytest.mark.asyncio
async def test_update_single_anime_vector_image_vector(client: QdrantClient, embedding_manager):
