...
Usage:
    ./pants run libs/enrichment/src/enrichment/crawlers/anisearch_episode_crawler.py -- <url> [--output PATH]
...
def _normalize_episode_url(anime_identifier: str) -> str:
    ...
    if (
        anime_identifier.startswith(("http://", "https://"))
        and "anisearch.com" not in anime_identifier
    ):
    ...
    if (
        ...
        and "/episodes" in anime_identifier
    ):
...
async def _fetch_anisearch_episodes_data(
