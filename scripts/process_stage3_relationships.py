...
def process_offline_urls(offline_urls: List[str], existing_urls: Set[str], existing_mal_base_urls: Set[str]) -> List[Dict]:
    ...
    print(f"  - Offline URL processing: {skipped_count} URLs skipped (already in Jikan/AnimePlanet)")
    print(f"  - Offline URL processing: {processed_count} URLs processed (all remaining URLs included)")
    return related_anime
        ...
        if len(entries) == 1:
            ...
            for entry in entries:
                ...
                if "myanimelist.net" in url:  # Jikan source
                    ...
                    best_entry = entry
                elif "animeschedule.net" in url and "anime-planet.com" not in best_entry.get("url", ""):
                    best_entry = entry
...
def process_all_relationships(current_anime_file: str, temp_dir: str):
    ...
    with open(f'{temp_dir}/jikan.json', 'r') as f:
        jikan_data = json.load(f)

    with open(f'{temp_dir}/anime_planet.json', 'r') as f:
        animeplanet_data = json.load(f)

    with open(f'{temp_dir}/animeschedule.json', 'r') as f:
        animeschedule_data = json.load(f)

    with open(f'{temp_dir}/anilist.json', 'r') as f:
    ...
    print(f"Jikan: {len(jikan_anime)} anime relations, {len(jikan_manga)} manga relations")
    ...
    print(f"AniList: {len(anilist_anime)} anime relations, {len(anilist_manga)} manga relations")
    ...
    offline_anime = process_offline_urls(offline_urls, existing_urls, existing_mal_base_urls)
    print(f"Offline URLs: {len(offline_urls)} URLs extracted, {len(offline_anime)} anime relations processed")

    # VERIFICATION: Confirm the counts match
    if len(offline_anime) == verification_processed:
        print(f"  ✅ VERIFICATION PASSED: Processed count matches ({len(offline_anime)})")
    else:
        print(f"  ❌ VERIFICATION FAILED: Expected {verification_processed}, got {len(offline_anime)}")
    ...
    # Create final output
    output = {
        "related_anime": all_anime_relations,
        "relations": all_manga_relations
    }
    ...
    print(f"Stage 3 processing complete:")
