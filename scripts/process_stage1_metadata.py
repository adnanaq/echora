...
def merge_themes_intelligently(
    ...
    for theme in external_themes:
        ...
        if normalized_name in themes_by_name:
            ...
            # New theme - add it
            themes_by_name[normalized_name] = {"name": name, "description": description}
...
def extract_trailers_with_deduplication(sources: dict[str, dict]) -> list[dict]:
    ...
    if youtube_id:
        ...
        if url not in seen_urls:
            trailers.append(
                {"url": url, "thumbnail_url": None, "title": "Official Trailer"}
            )
            seen_urls.add(url)
...
def extract_themes_from_sources(sources: dict[str, dict]) -> list[dict]:
    ...
    for tag in anilist.get("tags", []):
        if tag.get("name"):
            all_themes.append(
                {"name": tag["name"], "description": tag.get("description")}
            )
...
def extract_synonyms_from_sources(sources: dict[str, dict]) -> list[str]:
    ...
    kitsu = sources.get("kitsu", {})
    for title in (
        kitsu.get("data", {}).get("attributes", {}).get("abbreviatedTitles", [])
    ):
        if title:
...
def parse_theme_song_string(theme_string: str) -> dict[str, str | None]:
    ...
    if fallback_match:
        ...
        return {"title": title, "artist": artist if artist else None, "episodes": None}

    # Final fallback: try to extract at least the title
    if '"' in theme_string:
        title_match = re.search(r'"([^"]+)"', theme_string)
        if title_match:
            return {"title": title_match.group(1), "artist": None, "episodes": None}
    ...
    for source_name in source_hierarchy:
        ...
        if source_name == "jikan":
            ...
            if field == "episodes":
                ...
                value = (
                    media.get("duration") * 60 if media.get("duration") else None
...
def process_stage1_metadata(current_anime_file: str, temp_dir: str) -> dict[str, Any]:
    ...
    output["themes"] = merge_themes_intelligently(
        offline_themes, external_themes, existing_genres_set
    )
