...
def smart_merge(base: dict, other: dict) -> dict:
    ...
    for field in ("picture", "thumbnail", "status", "description"):
        ...
        if val_a in (None, "", "UNKNOWN") and val_b not in (None, "", "UNKNOWN"):
            base[field] = val_b
        elif field == "status" and val_a == "UNKNOWN" and val_b in ("FINISHED", "UPCOMING", "ONGOING"):
            base[field] = val_b
    ...
    for field in ("sources", "synonyms", "tags", "producers", "studios", "genres", "relatedAnime"):
    ...
    if season_a.get("season") in (None, "UNDEFINED") and season_b.get("season") not in (None, "UNDEFINED"):
...
def merge_database(file_path: str, output_path: str) -> None:
    ...
    def _merge_group(indices: list[int]) -> None:
        ...
        for dup_idx in indices[1:]:
            anime_list[base_idx] = smart_merge(anime_list[base_idx], anime_list[dup_idx])
